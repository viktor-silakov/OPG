...
class TextToSemantic(L.LightningModule):
    ...
    def forward(self, x):
        # Ensure input is float32 to prevent dtype mismatch with LoRA
    ...
    def _step(self, batch, batch_idx, stage: str):
        ...
        # Convert all batch tensors to float32 to prevent dtype mismatch
        for key in batch:
            if isinstance(batch[key], torch.Tensor) and batch[key].dtype == torch.bfloat16:
