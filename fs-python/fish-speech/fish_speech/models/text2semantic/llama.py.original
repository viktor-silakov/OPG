...
class BaseTransformer(nn.Module):
    ...
    def forward(
        ...
        for layer in self.layers:
            # Disable gradient checkpointing to prevent dtype mismatch with LoRA
    ...
    def from_pretrained(
        ...
        if load_weights is False:
            ...
            # Convert all weights to float32 to prevent dtype mismatch
            for k, v in weights.items():
                if v.dtype == torch.bfloat16:
                    weights[k] = v.float()
                    logger.info(f"Converted {k} from bfloat16 to float32")

            err = model.load_state_dict(weights, strict=False, assign=True)
            log.info(f"Loaded weights with error: {err}")
            
            # Ensure the entire model uses float32 to prevent LoRA dtype mismatch
            model = model.float()
            logger.info("Converted entire model to float32 for LoRA compatibility")
